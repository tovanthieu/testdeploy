...
if ($result_product->num_rows == 0) {
    // Không tìm thấy sản phẩm, chuyển hướng người dùng điều hướng lại
    header("Location: product.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['image']['size'] > 0) {
        // Nếu có, lưu file ảnh mới và cập nhật đường dẫn
        $target_dir = "anhsanpham/";
        $target_file = $target_dir . basename($_FILES["image"]["name"]);
        move_uploaded_file($_FILES["image"]["tmp_name"], $target_file);
        $image = basename($_FILES["image"]["name"]);
    } else {
        // Nếu không, sử dụng lại đường dẫn ảnh cũ
        $image = $product['image'];
    }
    ...
    $sql_update = "UPDATE product SET name = '$name', image = '$image', description = '$description', quantity = '$quantity', price = '$price', category_id = '$category_id' WHERE id = $product_id";
    if ($conn->query($sql_update) === TRUE) {
        echo "Cập nhật sản phẩm thành công!";
        // Sau khi cập nhật thành công, chuyển hướng người dùng về trang danh sách sản phẩm
        header("Location: product.php");
        exit();
    } else {
                ...
                <div class="container-fluid">
                    ...
                    <form action="<?php echo $_SERVER["PHP_SELF"] . "?id=$product_id"; ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <!-- Thêm thuộc tính accept="image/*" để chỉ cho phép người dùng chọn các file ảnh -->
                        </div>
                        <div class="form-group">
